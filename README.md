...
## رابط گرافیکی اختیاری جست‌وجو

برای دمو و بررسی دستی نتایج، یک UI محلی سبک هم وجود دارد. این بخش جزو الزام
صورت پروژه نیست و فقط ابزار کمکی برای کار عملی با indexهاست:
...
این UI بدون وابستگی اضافه ساخته شده و مستقیماً به Elasticsearch محلی وصل می‌شود.
می‌توانید بین baseline و optimized، حالت‌های keyword، phrase، contain و fuzzy،
تعداد نتیجه، دسته‌بندی، سال و عبارت حذف‌شونده جابه‌جا شوید.
